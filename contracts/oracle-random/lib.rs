...
pub mod oracle_anchor {
    ...
    impl RandomOracleGetter for RandomDataStorage {
        ...
        fn get_round(&self, round: u64) -> Option<(Vec<u8>, Vec<u8>, Vec<u8>)> {
            ...
            rds.value
                .get(round)
                .map(|data| (data.randomness, data.signature, data.previous_signature))
        }
    }
    ...
    //         let test_rounds = vec![
    //             (1, vec![2, 2, 3], vec![1, 2, 3], vec![1, 2, 3]),
    //             (2, vec![1, 2, 3], vec![4, 5, 6], vec![1, 2, 3]),
    //             (4, vec![1, 2, 3], vec![4, 5, 6], vec![1, 2, 3]),
    //             (3, vec![1, 2, 3], vec![4, 5, 6], vec![1, 2, 3]),
    //         ];
    ...
    //         assert_eq!(result, Some((vec![2, 2, 3], vec![1, 2, 3], vec![1, 2, 3])));
    //     }
